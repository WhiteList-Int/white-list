	...
	function handleChange(event) {
		...
		const newFilter = sampleData.filter((data) => {
			return data.category
				.toLowerCase()
				.includes(filterRental.current.toLowerCase());
		});
	...
	return (
		...
		>
			<section className='rental-dashboard-head'>
				...
				<div className='rental-nav-trans'>
					<NavbarTransparent />
				</div>
				<img
					className='rentel-dashboard-head-img'
					src={Img}
					alt='WhiteList'
				></img>
				<Select
					...
					className={
						windowState
							? 'rental-dashboard-filter-select-hide'
							: 'rental-dashboard-filter-select'
					}
				...
				<form
					className={
						windowState
							? 'rental-dashboard-filter-form'
							: 'rental-dashboard-filter-form-hide'
					}
				>
					...
					rawData={
						rentalFilter === 'all'
							...
							: sampleData.filter((data) => {
									return data.category
										.toLowerCase()
										.includes(rentalFilter.toLowerCase());
							  })
					}
					data={filteredData}
			...
			<section className='rental-dashboard-main-container'>
				...
				<div className='rental-dashboard-main-navbar'>
					<div className='rental-navbar-section'>
						<ArrowBackIcon
							className='rental-dashboard-back-icon'
								...
								<div className='main-rental-list-options'>
									...
									<div className='information-box'>
										<div className='stars-container'>
											<img
												src={rental.stars >= 1 ? starFilled : starUnfilled}
												alt='*'
											/>
											<img
												src={rental.stars - 1 >= 1 ? starFilled : starUnfilled}
												alt='*'
											/>
											<img
												src={rental.stars - 2 >= 1 ? starFilled : starUnfilled}
												alt='*'
											/>
											<img
												src={rental.stars - 3 >= 1 ? starFilled : starUnfilled}
												alt='*'
											/>
											<img
												src={rental.stars - 4 >= 1 ? starFilled : starUnfilled}
												alt='*'
											/>
										...
										<div className='price-box'>
											<div className='price-box-upper'>
												<h3>PHP</h3>
												<h2>{rental.minPrice.toFixed(2)}</h2>
											</div>
											<div className='price-box-lower'>
												<h6>
													{'~ Php ' +
														rental.maxPrice.toFixed(2) +
														'/' +
														rental.units}
												</h6>
											</div>
